...
(NAME, LASTNAME, GROUP, FACULTY, MEETING, DATE, GROUP_MEMBERS, WERE, WHY_NOT, HANDMAN, ORGANIZATIONS, WHERE_1, DOPS_1,
 ...
 PLUSES, MINUSES, TOTAL, COMMENTS) = range(27)
...
class GoogleSheetsManager:
        ...
        try:
            ...
            if not existing_data:
                self.headers = [
                    ...
                    "Выбор старосты",
                    #Информационная
                    "Организации", "Вопросы не по теме2",
                    #На сплочение
                    "Где была встреча1", "Использовали Коордбокс?1", "Чем занимались?1",
                    #Новогодняя предсессионная
                    "Вопросы не по теме1", "Тайный Санта",
                    #Информационная онлайн
                    "Какие моменты осветили?", "Вопросы не по теме3",
                    #Неформальная
                    "Где была встреча2", "Использовали Коордбокс?2", "Чем занимались?2",
                    #Итоговая
                    "Что обсуждали?",
                    "Плюсы встречи", "Минусы встречи", "Общие впечатления","Доп. инфа"
                ]
    ...
    def _add_data_sync(self, user_data: dict):
        ...
        column_mapping = {
            ...
            'Выбор старосты': user_data.get('Выбор старосты', ''),
            'Организации': user_data.get('Организации', ''),
            'Где была встреча1': user_data.get('Где была встреча1', ''),
            'Вопросы не по теме1': user_data.get('Вопросы не по теме1', ''),
            'Какие моменты осветили?': user_data.get('Какие моменты осветили?', ''),
            'Где была встреча2': user_data.get('Где была встреча2', ''),
            'Что обсуждали?': user_data.get('Что обсуждали?', ''),
            'Вопросы не по теме2': user_data.get('Вопросы не по теме2', ''),
            'Использовали Коордбокс?1': user_data.get('Использовали Коордбокс?1', ''),
            'Тайный Санта': user_data.get('Тайный Санта', ''),
            'Вопросы не по теме3': user_data.get('Вопросы не по теме3', ''),
            'Использовали Коордбокс?2': user_data.get('Использовали Коордбокс?2', ''),
...
async def get_comments(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if success:
        await update.message.reply_text(
            "Спасибо огромное! Я всё успешно записал)\n\nЕсли хочешь записать новую встречу, то напиши /again"
        )
...
async def again(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(
        'Новая встреча с твоими первашами?)\n\nНажимай /start и я всё запишу!',
        reply_markup=ReplyKeyboardRemove()
    )
